...
// @resource     popup_window https://raw.githubusercontent.com/moontai0724/UserScripts/develop/HideBlockList/popup_window.vue
// @resource     main.css https://raw.githubusercontent.com/moontai0724/UserScripts/develop/HideBlockList/main.css
...
(function (jQuery) {
    ...
    function restoreConfigHandler() {
        ...
        if (validateStructure(config, configFromUser) === false) {
    ...
    /**
     * Validate two data structure is same or not.
     * @param {JSON} source Source data to compare.
     * @param {JSON} target Target data to compare.
     * @returns {Boolean} Two data is compare or not
     */
    function validateStructure(source, target) {
        let isSame = false;
        if (typeof source === "object" && isJSON(source) && isJSON(target)) {
            for (let index in source) {
                if (typeof source[index] !== typeof target[index] || (typeof source[index] === "object" && !(isJSON(source[index]) === isJSON(target[index])))) {
                    isSame = true;
                    break;
                }

                if (!isSame && typeof source[index] === "object" && isJSON(source[index]) && isJSON(target[index])) {
                    isSame = isSame || !validateStructure(source[index], target[index]);
                }
            }
        }
        return !isSame;
